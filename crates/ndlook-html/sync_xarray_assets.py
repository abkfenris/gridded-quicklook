...
ndlook-html's renderer emits markup that mirrors xarray's `_repr_html_`
...
writes them into `crates/ndlook-html/assets/`, prepending an attribution
header to the CSS.

Run with:

    ./crates/ndlook-html/sync_xarray_assets.py

(or: uv run --script crates/ndlook-html/sync_xarray_assets.py, or via
 ...
 * here so ndlook's HTML Quick Look renderer -- whose emitted
