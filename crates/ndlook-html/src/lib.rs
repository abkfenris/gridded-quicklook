//! `ndlook-html` renders the metadata produced by `ndlook-meta` into an
...
use ndlook_meta::model::{DatasetSummary, SourceFormat, VersionInfo};

/// xarray's HTML repr stylesheet, copied verbatim (see
/// `assets/xarray-style.css` for attribution) via `mise run
/// sync-xarray-assets` / `crates/ndlook-html/sync_xarray_assets.py`.
