...
fn flat_dataset() -> DatasetSummary {
    let root = GroupSummary {
        ...
        attrs: vec![
            (
                "title".to_string(),
                AttrValue::Text("ndlook simple fixture".to_string()),
            ),
            (
...
fn flat_dataset_snapshot() {
    let ds = flat_dataset();
    let html = ndlook_html::render_page(&ds, "simple.nc", Some(236));
...
fn nested_tree_snapshot() {
    let ds = nested_tree();
    let html = ndlook_html::render_page(&ds, "tree.zarr", Some(4096));
    insta::assert_snapshot!(html);
}
...
fn versioned_dataset_snapshot() {
    ...
    let html = ndlook_html::render_page(&ds, "icechunk_repo", None);
    insta::assert_snapshot!(html);
}
...
fn empty_dataset_snapshot() {
    ...
    let html = ndlook_html::render_page(&ds, "empty.zarr", Some(0));
    insta::assert_snapshot!(html);
}
