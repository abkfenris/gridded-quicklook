...
gridded-html's renderer emits markup that mirrors xarray's `_repr_html_`
...
writes them into `crates/gridded-html/assets/`, prepending an attribution
header to the CSS.

Run with:

    ./crates/gridded-html/sync_xarray_assets.py

(or: uv run --script crates/gridded-html/sync_xarray_assets.py, or via
 ...
 * here so gridded-quicklook's HTML QuickLook renderer -- whose emitted
