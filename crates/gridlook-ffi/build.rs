//! Generates `apple/include/gridlook_ffi.h` (gitignored) from this crate's
...
fn main() {
    ...
    let config = match cbindgen::Config::from_file(crate_dir.join("cbindgen.toml")) {
        Ok(config) => config,
        Err(err) => {
            println!("cargo:warning=gridlook-ffi: failed to read cbindgen.toml: {err}");
            return;
        }
    };
        ...
        .generate()
    {
        ...
        Err(err) => {
            ...
            println!("cargo:warning=gridlook-ffi: failed to generate C header: {err}");
            return;
        }
    };
