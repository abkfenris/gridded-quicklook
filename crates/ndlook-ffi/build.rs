//! Generates `apple/include/ndlook_ffi.h` (gitignored) from this crate's
...
fn main() {
    ...
    let config = match cbindgen::Config::from_file(crate_dir.join("cbindgen.toml")) {
        Ok(config) => config,
        Err(err) => {
            println!("cargo:warning=ndlook-ffi: failed to read cbindgen.toml: {err}");
            return;
        }
    };
        ...
        .generate()
    {
        ...
        Err(err) => {
            ...
            println!("cargo:warning=ndlook-ffi: failed to generate C header: {err}");
            return;
        }
    };
    ...
    {
        println!("cargo:warning=ndlook-ffi: cannot create apple/include: {err}");
        return;
    }
    if let Err(err) = fs::write(&header_path, new_contents) {
        println!("cargo:warning=ndlook-ffi: cannot write apple/include/ndlook_ffi.h: {err}");
    }
}
