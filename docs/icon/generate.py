...
def icon(bare: bool = False, small: bool = False, with_loupe: bool = True, letters: bool = True) -> str:
    ...
    title = "ndLook icon" + (" (bare)" if bare else "") + (" (small sizes)" if small else "")
    if not with_loupe:
        title = "ndLook document badge"
    out = [svg_open(title)]
...
OUTPUTS = {
    "ndlook": lambda: icon(),
    "ndlook-small": lambda: icon(small=True),
    "ndlook-bare": lambda: icon(bare=True),
    # The badge is composed onto a document shape and shown small, so it
    # uses the reduced (2x2 grid, two-band) styling.
    "ndlook-badge": lambda: icon(bare=True, small=True, with_loupe=False),
}
