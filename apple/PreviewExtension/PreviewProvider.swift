...
//  `ndlook-ffi`, the Rust core) and hand back the bytes directly.
//
...
private let logger = Logger(subsystem: "com.alexkerney.ndlook", category: "preview")
...
final class PreviewProvider: QLPreviewProvider, QLPreviewingController {
    ...
    func providePreview(for request: QLFilePreviewRequest) async throws -> QLPreviewReply {
        ...
        //
        // We don't use it: ndlook-ffi renders straight to an in-memory
        // string, so there's no intermediate file to hand QuickLook.
    }

    /// Calls into `ndlook-ffi`'s C ABI to render `url` as a complete HTML
    /// document. `ndlook_render_html` never fails in the FFI sense -- on
    ...
    private func renderHTML(forFileAt url: URL) -> String {
        url.withUnsafeFileSystemRepresentation { fsPath -> String in
            ...
            guard let cString = ndlook_render_html(fsPath) else {
                return Self.fallbackErrorHTML
            }
    ...
    /// Used only if `ndlook_render_html` itself returns NULL, which its
    ...
    private static let fallbackErrorHTML =
        "<!doctype html><html><body><p>Preview unavailable: ndlook-ffi returned no data.</p></body></html>"
}
